...
from model.database_util import get_hist_file, get_job_table_sample, collator #获取直方图文件和获取表样本 与数据库统计/采样/数据整理相关工具
from model.model_transformer_qf import QueryFormer #模型类（这是脚本要训练的模型）
from model.database_util import Encoding #编码相关类（可能用于把 plan 元素编码成索引/embedding）
from model.dataset_qf import PlanTreeDataset #把 plan 数据组织成 PyTorch Dataset 的类
from model.trainer_cost_job import evaluate, train #训练与评估工作流函数
...
# 可选加载预训练 cost 模型（若存在）。当前脚本未强制使用该 checkpoint。
# cost_ckpt_path = 'checkpoints/cost_model.pt'
# if os.path.exists(cost_ckpt_path):
#     try:
#         checkpoint = torch.load(cost_ckpt_path, map_location='cpu')
#         print(f"[Info] Found and loaded optional cost checkpoint: {cost_ckpt_path}")
#     except Exception as e:
#         print(f"[Warn] 加载 {cost_ckpt_path} 失败：{e}")
...
for i in range(1, 2):
    # file = imdb_path + 'plan_and_cost/update_train_plan_part{}.csv'.format(i)
    # file = imdb_path + 'plan_and_cost/mytest.csv'.format(i)
...
crit = nn.MSELoss()
# 调用 train 函数开始训练过程。返回两个值：
# model：训练后的模型（可能已经在内部做 .to() / checkpoint 保存 / 恢复）。
# best_path：训练期间保存的最优模型路径（checkpoint 文件路径），便于后续加载或评估
# 在加载 encoding 后添加这些调试代码
# print("=== 详细调试 encoding 对象 ===")
# print(f"Encoding 类型: {type(encoding)}")
# print(f"Encoding 属性: {dir(encoding)}")
# # 检查所有属性
...
best_res.to_csv('results/job-full/cost/cost-qf-job.csv')