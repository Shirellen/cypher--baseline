...
class FeatureEmbed(nn.Module):
    # ！！！！！！！！！！！job 21张表
    def __init__(self, embed_size=32, tables = 21, types=39, joins = 55, columns= 61, ops=4, use_sample = True, use_hist = True, use_schema = True, bin_number = 50, subschema_dim=256):
        ...
        self.subschema_dim = subschema_dim
        ...
        # 新增：subschema 投影层（把多热 subschema 向量映射到 embed_size）
        self.linearSubschema = nn.Linear(self.subschema_dim, embed_size)
        # if use_schema:
        #     self.project = nn.Linear(embed_size*6 + embed_size//8+1, embed_size*6 + embed_size//8+1)
        # else:
        #     self.project = nn.Linear(embed_size*5 + embed_size//8+1, embed_size*5 + embed_size//8+1)

        # 现在有6个组件: type + filter + join + table + subschema + hist(可选)
        if use_hist:
            self.project = nn.Linear(embed_size*6 + embed_size//8+1, embed_size*6 + embed_size//8+1)
        else:
            self.project = nn.Linear(embed_size*5 + embed_size//8+1, embed_size*5 + embed_size//8+1)
    ...
    def forward(self, feature):
        # LDBC label有14+1('NA')个，所以table_sample=15+1000
        # typeId, joinId, filtersId, filtersMask, hists, table_sample= torch.split(feature,(1,1,9,3,self.bin_number*3,1000+15), dim = -1)
        # ！！！！！！！！！！！！！！job是21张表
        typeId, joinId, filtersId, filtersMask, hists, table_sample, subschema= torch.split(feature,(1,1,9,3,self.bin_number*3,1+21,256), dim = -1)
        # typeId, joinId, filtersId, filtersMask, hists, table_sample, subschema= torch.split(feature,(1,1,9,3,self.bin_number*3,1000+15,256), dim = -1)
        ...
        # 将子schema(256维)投影到 embed_size，确保与 project 输入维度一致
        subschemaEmb = self.linearSubschema(subschema)

        if self.use_hist:
            final = torch.cat((typeEmb, filterEmbed, joinEmb, tableEmb, subschemaEmb, histEmb), dim = 1)
        else:
            final = torch.cat((typeEmb, filterEmbed, joinEmb, tableEmb, subschemaEmb), dim = 1)
        # if self.use_schema:
        #     final = torch.cat((typeEmb, filterEmbed, joinEmb, tableEmb, subschemaEmb, histEmb), dim = 1)
        # else:
    ...
    def getTable(self, table_sample):
        # table, sample = torch.split(table_sample,(15,1000), dim = -1)
        # ！！！！！！！！job 21张表
        table, sample = torch.split(table_sample,(21,1), dim = -1)
        # emb = self.tableEmbed(table.long()).squeeze(1)

        # table是多热编码 [batch, 15]，需要加权平均或求和
        table_emb = self.tableEmbed.weight  # [15, 64]
        
        # 加权平均（推荐）
        table_weights = F.softmax(table, dim=-1)  # 归一化权重
        emb = torch.matmul(table_weights, table_emb)  # [batch, 64]
    ...
    def getHist(self, hists, filtersMask):
        ...
        num_filters = torch.sum(filtersMask,dim = 1).clamp(min=1)
...
class QueryFormer(nn.Module):
    def __init__(self, emb_size = 32 ,ffn_dim = 32, head_size = 8, \
        ...
        # 如果不需要 subschema，则 hidden_dim 少 emb_size
        # 现在有6个组件: type + filter + join + table + subschema（可选） + hist
        # if use_schema:
        #     hidden_dim = emb_size * 6 + emb_size //8 + 1
        # else:
        #     hidden_dim = emb_size * 5 + emb_size //8 + 1
    ...
    def forward(self, batched_data):
        ...
        # x_view = x.view(-1, 1179)
        x_view = x.view(-1, 442)
        # x_view = x.view(-1, 1435)
